...
(define-fun T-par-x () Bool (forall ((C Term) (T Term)) (=> (tcheck C const-xvar T) (tcheck C const-xvar T))))
(define-fun T-par-app () Bool (forall ((C Term) (T Term)) (=> (tcheck C (app const-eapp1 const-eapp2) T) (tcheck C (app (join (fork (par const-eapp1))) (join (fork (par const-eapp2)))) T))))
(define-fun T-par-abs () Bool (forall ((C Term) (T Term)) (=> (tcheck C (fabs const-xabs const-sabs const-eabs) T) (tcheck C (fabs const-xabs const-sabs (par const-eabs)) T))))
(push 1)
(assert (not T-par-x))
(check-sat)
(pop 1)
