...
for (t in c("10s", "30s", "60s", "120s")) {
    ...
    boxplot1topdf(paste(t, "/", "Graph2/princess-successrate_per_typingconfiguration.csv", sep=""), title="Princess CASC version", ylab="Success rate (%) for compilation strategy")
...
for (t in c("10s", "30s", "60s", "120s")) {
    ...
    boxplot1topdf(paste(t, "/", "Graph3/princess-successrate_per_variableconfiguration.csv", sep=""), title="Princess CASC version", ylab="Success rate (%) for compilation strategy")
...
for (t in c("10s", "30s", "60s", "120s")) {
    boxplot1topdf(paste(t, "/", "Graph4/eprover-successrate_per_simplificationconfiguration.csv", sep=""), title="Eprover", ylab="Success rate (%) for compilation strategy")
    boxplot1topdf(paste(t, "/", "Graph4/princess-successrate_per_simplificationconfiguration.csv", sep=""), title="Princess CASC version", ylab="Success rate (%) for compilation strategy")
    boxplot1topdf(paste(t, "/", "Graph4/vampire-3.0-successrate_per_simplificationconfiguration.csv", sep=""), title="Vampire 3.0", ylab="Success rate (%) for compilation strategy")
    boxplot1topdf(paste(t, "/", "Graph4/vampire-4.0-successrate_per_simplificationconfiguration.csv", sep=""), title="Vampire 4.0", ylab="Success rate (%) for compilation strategy")
}

### Boxplots for fifth paper graph (simplification alternatives, timeout-wise, all categories and all provers)
for (t in c("10s", "30s", "60s", "120s")) {
    boxplot1topdf(paste(t, "/", "Graph5/simplificationperformance_allprovers_allcategories.csv", sep=""), title=t, ylab="Success rate (%) for compilation strategy")
}

### Boxplots for sixth paper graph (performance of all comp strategies, all provers and categories together)
for (t in c("10s", "30s", "60s", "120s")) {
    boxplot1topdf(paste(t, "/", "Graph6/stratperformance_allprovers_allcategories.csv", sep=""), ylab="Success rate (%) for compilation strategy", las=2) 
}
