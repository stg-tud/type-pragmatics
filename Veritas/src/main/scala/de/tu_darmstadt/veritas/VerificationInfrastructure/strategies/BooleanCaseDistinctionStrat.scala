...
case class BooleanCaseDistinctionStrat[Def, Formulae <: Def](poscond: Formulae, spec_enquirer: SpecEnquirer[Def, Formulae], fc_calls: Seq[String], additional_premises: Seq[Formulae] = Seq())
  ...
  override def applyToPG(pg: ProofGraph[Def, Formulae] with ProofGraphTraversals[Def, Formulae])(obl: pg.Obligation): ProofGraph[Def, Formulae] with ProofGraphTraversals[Def, Formulae] = {
    val bcase_tac = BooleanCaseDistinction[Def, Formulae](poscond, spec_enquirer, fc_calls, additional_premises)
