...
/**
  * abstract type of nodes in a proof graph, represents a single subgoal/step in a proof
  *
  * @param name name of the proof step (should be unique)
  */
abstract class ProofStep[S, P](val name: String) {
  /**
    * every node can dynamically compute via this method whether
    * he is fully verified or not
    */
  def fullyVerified(): Boolean
}
...
/**
  ...
  * @param edges edges to child proof steps
...
class SingleProofStep[S, P](name: String,
  ...
  /**
    ...
    * he is fully verified or not
    */
  override def fullyVerified(): Boolean = ???
}
...
/**
  ...
  * @tparam P type of the format for defining properties
  */
class LeafProofStep[S, P](name: String, spec: S, goal: P)
  extends SingleProofStep[S, P](name, spec, goal, Seq()) with Verifiable[S, P] {
  /**
    * every node can dynamically compute via this method whether
    * he is fully verified or not
    */
  override def fullyVerified(): Boolean = this.getVerificationStatus.isVerified

  override def verify(verifier: Verifier[S, P]): LeafProofStep[S, P] =
    new LeafProofStep[S, P](name, spec, goal) {
      override val verificationStatus: VerificationStatus = verifier.verify(spec, Seq(), goal, Solve)
    }

  //TODO: this method probably requires a couple of parameters
  override def makeOutdated(): LeafProofStep[S, P] = ???
}

/**
  *
  * @param name     name of the proof step (should be unique)
  * @param siblings other proof steps that belong to the composed proof step;
  *                 these steps must be SingleProofSteps, i.e. no nesting of ComposedProofSteps is
  *                 allowed!
  * @tparam S type of the specification format
  * @tparam P type of the format for defining properties
  */
case class ComposedProofStep[S, P](override val name: String, siblings: Seq[SingleProofStep[S, P]])
  extends ProofStep(name) {
  /**
    * every node can dynamically compute via this method whether
    * he is fully verified or not
