...
import de.tu_darmstadt.veritas.scalaspl.util.{Reporter, ScalaMetaUtils}
...
trait AlgebraicDataTypeTranslator {
  ...
  private def checkBaseTraitSuperType(supertypes: Seq[String]): Boolean = supertypes.forall(!_.contains(ScalaSPLTranslator.predefTraits))
